    ...
    <Form>
      <Form-item>
        <Form inline>
          ...
          <Modal :mask-closable="false" title="发货确认" width="400" v-model="viewData.modalSend" @on-ok="onSend">
            <Form :label-width="80">
              <Form-item class="form_item" label="订单号:">
                <Input class="search_item" disabled type="text" v-model="viewData.Confirm.id" clearable
                  placeholder="订单ID"></Input>
              </Form-item>
          ...
          <Modal :mask-closable="false" title="付款确认" width="400" v-model="viewData.modalPay" @on-ok="onPay">
            确认订单id：
          ...
          <Modal title="查看订单信息详情" width="55" :styles="{top: '70px'}" v-model="viewData.modalDetail">
            <div class="order_info">
              ...
              <h4>收货地址: {{viewData.Detail.contactAddress}}</h4>
              <Row>
                <Col span="10">创建时间: {{viewData.Detail.createTime}}</Col>
                <Col span="10">付款时间: {{viewData.Detail.payTime}}</Col>
              </Row>
              <Row>
                <Col span="10">是否需要运费: {{viewData.Detail.isNeedFreight}}</Col>
    ...
    return {
      ...
      searchList: {
        ...
        },
        {
          ...
          key: 'isNeedFreight',
          render (h, params) {
            return h('span', params.row.isNeedFreight ? '需要' : '包邮')
          }
        },
        {
          title: '订单状态',
